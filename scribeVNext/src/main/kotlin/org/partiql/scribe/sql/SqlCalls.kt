...
public abstract class SqlCalls(context: ScribeContext) {
    ...
    public open fun unary(op: String, args: SqlArgs): Expr {
        if (args.size != 1) {
            listener.reportAndThrow(ScribeProblem.simpleError(ScribeProblem.INVALID_PLAN, "Unary operator '$op' requires exactly 1 argument"))
        }
        return exprOperator(symbol = op, lhs = null, rhs = args[0].expr)
    }

    public open fun binary(op: String, args: SqlArgs): Expr {
        if (args.size != 2) {
            listener.reportAndThrow(ScribeProblem.simpleError(ScribeProblem.INVALID_PLAN, "Binary operator '$op' requires exactly 2 arguments"))
        }
    ...
    public open fun notFn(args: SqlArgs): Expr {
        ...
        return when (arg) {
            ...
            is ExprInCollection -> {
                val collection = when (val coll = arg.rhs) {
    ...
    public open fun inCollection(args: SqlArgs): Expr {
        val collection = when (val arg1 = args[1].expr) {
     ...
     */
    public open fun trim(args: SqlArgs, spec: TrimSpec): Expr {
        return when (args.size) {
            ...
            else -> listener.reportAndThrow(ScribeProblem.simpleError(ScribeProblem.INVALID_PLAN, "Unsupported trim(...) with arity ${args.size}"))
        }
    }
}
