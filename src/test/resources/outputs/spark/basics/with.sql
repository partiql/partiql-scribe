...
WITH `cte1` AS (SELECT `SIMPLE_T`.`a` AS `a`, `SIMPLE_T`.`b` AS `b` FROM `default`.`SIMPLE_T` AS `SIMPLE_T`), `cte2` AS (SELECT `cte1`.`a` AS `a` FROM `cte1` AS `cte1`), `cte3` AS (SELECT `cte2`.`a` AS `a` FROM `cte2` AS `cte2`) SELECT `cte1`.`a` AS `a`, `cte1`.`b` AS `b`, `cte2`.`a` AS `a`, `cte3`.`a` AS `a` FROM `cte1` AS `cte1` INNER JOIN `cte2` AS `cte2` ON `cte1`.`a` = `cte2`.`a` INNER JOIN `cte3` AS `cte3` ON `cte2`.`a` = `cte3`.`a`;
