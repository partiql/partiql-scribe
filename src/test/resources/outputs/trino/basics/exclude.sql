...
--#[exclude-07]
SELECT "$__EXCLUDE_ALIAS__".t.* FROM (SELECT (SELECT "transform"("t".a, coll_wildcard -> CAST(ROW(coll_wildcard.field_y) AS ROW(field_y VARCHAR))) AS a, "t".foo AS foo) AS "t" FROM "default"."EXCLUDE_T_COLL_WILDCARD" AS "t") AS "$__EXCLUDE_ALIAS__";

--#[exclude-08]
SELECT "$__EXCLUDE_ALIAS__".t.* FROM (SELECT (SELECT "transform"("t".a, coll_wildcard -> CAST(ROW(coll_wildcard.field_x) AS ROW(field_x INTEGER))) AS a, "t".foo AS foo) AS "t" FROM "default"."EXCLUDE_T_COLL_WILDCARD" AS "t") AS "$__EXCLUDE_ALIAS__";
