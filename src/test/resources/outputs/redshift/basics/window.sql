...
--#[window-15]
SELECT "T"."a", ROW_NUMBER() OVER "w" AS "_1" FROM "default"."T" AS "T" WINDOW "w" AS (ORDER BY "T"."a" ASC NULLS LAST);

--#[window-16]
SELECT "T"."a", RANK() OVER "w1" AS "_1", DENSE_RANK() OVER "w2" AS "_2" FROM "default"."T" AS "T" WINDOW "w1" AS (ORDER BY "T"."a" ASC NULLS LAST), "w2" AS (ORDER BY "T"."b" DESC NULLS FIRST);

--#[window-17]
SELECT "T"."a", ROW_NUMBER() OVER "w" AS "_1" FROM "default"."T" AS "T" WINDOW "w" AS (PARTITION BY "T"."b" ORDER BY "T"."a" ASC NULLS LAST);

-- Multiple window functions with named windows
--#[window-18]
SELECT "T"."a", "T"."b", ROW_NUMBER() OVER "w1" AS "rn1", RANK() OVER "w1" AS "rank1", LAG("T"."a", 1) RESPECT NULLS OVER "w1" AS "lag1" FROM "default"."T" AS "T" WINDOW "w1" AS (PARTITION BY "T"."b" ORDER BY "T"."a" ASC NULLS LAST);

-- Complex window with multiple partitions and orders
--#[window-19]
SELECT "T"."a", "T"."b", "T"."c", ROW_NUMBER() OVER "w1" AS "rn", RANK() OVER "w2" AS "rank_desc" FROM "default"."T" AS "T" WINDOW "w1" AS (PARTITION BY "T"."a" ORDER BY "T"."b" ASC NULLS LAST, "T"."c" ASC NULLS LAST), "w2" AS (PARTITION BY "T"."a" ORDER BY "T"."b" DESC NULLS FIRST, "T"."c" DESC NULLS FIRST);
