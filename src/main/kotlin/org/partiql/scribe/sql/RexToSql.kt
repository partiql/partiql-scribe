...
public open class RexToSql(
    ...
    override fun visitRexOpCollection(node: Rex.Op.Collection, ctx: StaticType): Expr {
        val type = staticTypeToCollectionType(ctx)
        val values = node.values.map { visitRexOp(it.op, it.type) }
        return exprCollection(type, values)
    }

    private fun staticTypeToCollectionType(type: StaticType): Expr.Collection.Type {
        return when (type.asNullable()) {
            is ListType -> Expr.Collection.Type.LIST
            is SexpType -> Expr.Collection.Type.SEXP
            is BagType -> Expr.Collection.Type.BAG
            else -> error("Type is not a collection type, found: $type")
        }
    }
