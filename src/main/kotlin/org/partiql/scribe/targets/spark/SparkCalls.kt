...
//        log.info("PartiQL `date_add($part, <quantity>, <date>)` was replaced by Spark `<date> + $intervalString`.")
     ...
     */
    override fun dateDiff(part: DatetimeField, args: SqlArgs): Expr = when (part.code()) {
        DatetimeField.YEAR -> {
            val d1 = args[0].expr
            val d2 = args[1].expr
            val call = exprCall(
...
//            log.info("PartiQL `date_diff(second, <date_1>, <date_2>)` was replaced by Spark `unix_timestamp(<date_2>) - unix_timestamp(<date_1>)`.")
    ...
    private fun diff(lhs: Expr, rhs: Expr) = exprOperator(
        symbol = "-",
        lhs = lhs,
        rhs = rhs,
    )

    private fun div(arg: Expr, divisor: Int) = exprOperator(
        symbol = "/",
        lhs = arg,
