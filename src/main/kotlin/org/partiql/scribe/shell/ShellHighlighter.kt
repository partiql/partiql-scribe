...
private val STYLE_COMMAND = AttributedStyle.DEFAULT
private val STYLE_KEYWORD = AttributedStyle.BOLD
private val STYLE_DATATYPE = AttributedStyle.DEFAULT
private val STYLE_IDENTIFIER = AttributedStyle.DEFAULT
private val STYLE_STRING = AttributedStyle.DEFAULT
private val STYLE_NUMBER = AttributedStyle.DEFAULT
private val STYLE_COMMENT = AttributedStyle.DEFAULT.foreground(AttributedStyle.BRIGHT).italic()
private val STYLE_ERROR = AttributedStyle.DEFAULT

/**
 * This only bolds keywords because it can be unreadable depending on a user's terminal settings.
 */
internal class ShellHighlighter : Highlighter {
    ...
    private class RethrowErrorListener : BaseErrorListener() {
        ...
        class OffendingSymbolException(val offendingSymbol: org.partiql.parser.thirdparty.antlr.v4.runtime.Token) :
            Exception()
    }
    ...
    private fun isKeyword(type: Int, text: String): Boolean =
        PartiQLTokens.VOCABULARY.getSymbolicName(type).equals(text, ignoreCase = true)
    ...
    private fun isDatatype(type: Int) = when (type) {
        ...
        PartiQLTokens.BOOL, PartiQLTokens.BOOLEAN, PartiQLTokens.ANY,
        -> true
        else -> false
    }
    ...
    private fun isIdentifier(type: Int) = when (type) {
        ...
        PartiQLTokens.IDENTIFIER_QUOTED,
        -> true
        else -> false
    }
